...
def get_next_bandit():
    ...
    for bnd in bandit_dict:
        expect = (bandit_dict[bnd]['win'] - bandit_dict[bnd]['loss'] + bandit_dict[bnd]['opp'] - (
...
def multi_armed_probabilities(observation, configuration):
    ...
    if 0 == observation['step']:
        ...
        if 0 < last_reward:
            bandit_dict[observation['lastActions'][my_idx]]['win'] = bandit_dict[observation['lastActions'][my_idx]][
                                                                         'win'] + 1
        else:
            bandit_dict[observation['lastActions'][my_idx]]['loss'] = bandit_dict[observation['lastActions'][my_idx]][
                                                                          'loss'] + 1
        bandit_dict[observation['lastActions'][1 - my_idx]]['opp'] = \
        bandit_dict[observation['lastActions'][1 - my_idx]]['opp'] + 1
